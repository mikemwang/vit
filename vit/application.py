...
class Application():
    ...
    def set_report(self):
        if len(self.extra_filters) == 0:
            ...
            self.report = self.config.get(
                'report', 'default_filter_only_report')

    def setup_main_loop(self):
        self.loop = urwid.MainLoop(urwid.Text(
            ''), unhandled_input=self.key_pressed, pop_ups=True, handle_mouse=self.config.mouse_enabled)
        try:
    ...
    def bootstrap(self, load_early_config=True):
        ...
        self.keybinding_parser = KeybindingParser(
            self.loader, self.config, self.action_registry)
        self.command = Command(self.config)
        self.get_available_task_columns()
        self.setup_keybindings()
        self.action_manager = ActionManagerRegistry(
            self.action_registry, self.key_cache.keybindings, event=self.event)
        self.register_managed_actions()
        ...
        self.task_color_config = TaskColorConfig(
            self.config, self.task_config, self.theme, self.theme_alt_backgrounds)
        self.init_task_colors()
        self.task_colorizer = TaskColorizer(self.task_color_config)
        self.formatter = FormatterBase(
            self.loader, self.config, self.task_config, self.markers, self.task_colorizer)
        ...
        ), event=self.event, request_reply=self.request_reply, action_manager=self.action_manager)
        self.event.listen('command-bar:keypress', self.command_bar_keypress)
        self.event.listen('task:denotate', self.denotate_task)
        self.event.listen('action-manager:action-executed',
                          self.action_manager_action_executed)
    ...
    def register_managed_actions(self):
        ...
        self.action_manager_registrar.register(
            'QUIT_WITH_CONFIRM', self.activate_command_bar_quit_with_confirm)
        ...
        self.action_manager_registrar.register('REFRESH', self.refresh)
        self.action_manager_registrar.register(
            'TASK_ADD', self.activate_command_bar_add)
        self.action_manager_registrar.register(
            'REPORT_FILTER', self.activate_command_bar_filter)
        ...
        self.action_manager_registrar.register(
            'COMMAND_BAR_EX', self.activate_command_bar_ex)
        self.action_manager_registrar.register(
            'COMMAND_BAR_EX_TASK_READ_WAIT', self.activate_command_bar_ex_read_wait_task)
        self.action_manager_registrar.register(
            'COMMAND_BAR_SEARCH_FORWARD', self.activate_command_bar_search_forward)
        self.action_manager_registrar.register(
            'COMMAND_BAR_SEARCH_REVERSE', self.activate_command_bar_search_reverse)
        self.action_manager_registrar.register(
            'COMMAND_BAR_SEARCH_NEXT', self.activate_command_bar_search_next)
        self.action_manager_registrar.register(
            'COMMAND_BAR_SEARCH_PREVIOUS', self.activate_command_bar_search_previous)
        self.action_manager_registrar.register(
            'COMMAND_BAR_TASK_CONTEXT', self.activate_command_bar_task_context)
        self.action_manager_registrar.register(
            'GLOBAL_ESCAPE', self.global_escape)
        self.action_manager_registrar.register(
            self.action_registry.noop_action_name, self.action_registry.noop)
        # Task.
        self.action_manager_registrar.register(
            'TASK_ANNOTATE', self.task_action_annotate)
        self.action_manager_registrar.register(
            'TASK_DELETE', self.task_action_delete)
        self.action_manager_registrar.register(
            'TASK_DENOTATE', self.task_action_denotate)
        self.action_manager_registrar.register(
            'TASK_MODIFY', self.task_action_modify)
        self.action_manager_registrar.register(
            'TASK_START_STOP', self.task_action_start_stop)
        self.action_manager_registrar.register(
            'TASK_DONE', self.task_action_done)
        self.action_manager_registrar.register(
            'TASK_PRIORITY', self.task_action_priority)
        self.action_manager_registrar.register(
            'TASK_PROJECT', self.task_action_project)
        self.action_manager_registrar.register(
            'TASK_TAGS', self.task_action_tags)
        self.action_manager_registrar.register(
            'TASK_WAIT', self.task_action_wait)
        self.action_manager_registrar.register(
            'TASK_EDIT', self.task_action_edit)
        self.action_manager_registrar.register(
            'TASK_SHOW', self.task_action_show)
            ...
            if task and task[attribute]:
                if type(task[attribute]) in ['set', 'tuple', 'dict', 'list']:
                    try:
                        ...
                        raise RuntimeError(
                            'Error parsing task attribute %s as JSON: %s' % (task[attribute], e))
    ...
    def add_user_keybinding_replacements(self, replacements):
        klass = self.loader.load_user_class(
    ...
    def wrap_replacements_callbacks(self, replacements):
        ...
        for i, replacement in enumerate(replacements):
            replacements[i]['replacement_callback'] = build_wrapper(
                replacement['replacement_callback'])
    ...
    def setup_keybindings(self):
        ...
        replacements = self.wrap_replacements_callbacks(
            self.add_user_keybinding_replacements(self.default_keybinding_replacements()))
        keybindings = self.keybinding_parser.add_keybindings(
            bindings=bindings, replacements=replacements)
        self.key_cache = KeyCache(keybindings)
        self.key_cache.build_multi_key_cache()

    def set_request_callbacks(self):
        self.request_reply.set_handler(
            'application:keybindings', 'Get keybindings', lambda: self.keybinding_parser.keybindings)
        self.request_reply.set_handler(
            'application:key_cache', 'Get key cache', lambda: self.key_cache)
        self.request_reply.set_handler(
            'application:blocking_task_uuids', 'Get blocking task uuids', lambda: self.blocking_task_uuids)
    ...
    def check_macro(self, keys):
        keybinding = self.keybinding_parser.keybindings[
            keys] if keys in self.keybinding_parser.keybindings else False
    ...
    def denotate_task(self, data):
        ...
        if task:
            ...
            self.activate_message_bar(
                'Task %s denotated' % self.model.task_id(task['uuid']))
            self.task_list.focus_by_task_uuid(
                data['uuid'], self.previous_focus_position)
    ...
    def command_bar_keypress(self, data):
        ...
        if 'choices' in data['metadata']:
            ...
            if op == 'quit' and choice:
                ...
                if task:
                    self.table.flash_focus()
                    self.update_report()
                    self.activate_message_bar('Task %s priority set to: %s' % (
                        self.model.task_id(task['uuid']), task['priority'] or 'None'))
            ...
            if op == 'ex':
                ...
                if task:
                    ...
                    self.update_report()
                    self.activate_message_bar(
                        'Task %s project updated' % self.model.task_id(task['uuid']))
            elif op == 'wait':
                # TODO: Validation if more than one arg passed.
                ...
                # modified.
                returncode, stdout, stderr = self.command.run(
                    ['task', 'rc.recurrence.confirmation=no', metadata['uuid'], 'modify', 'wait:%s' % wait], capture_output=True)

                if returncode == 0:
                    self.table.flash_focus()
                    self.update_report()
                    self.activate_message_bar(
                        'Task %s wait updated' % self.model.task_id(metadata['uuid']))
                else:
                    self.activate_message_bar(
                        "Error setting wait: %s" % stderr, 'error')
            elif op == 'context':
                # TODO: Validation if more than one arg passed.
                ...
                if self.execute_command(['task', 'context', context], wait=self.wait):
                    self.activate_message_bar(
                        'Context switched to: %s' % context)
                else:
                    self.activate_message_bar(
                        'Error switching context', 'error')
            elif len(args) > 0:
                if op == 'add':
                    if self.execute_command(['task', 'add'] + args, wait=self.wait):
                        task = self.task_get_latest()
                        self.activate_message_bar(
                            'Task %s added' % task_id_or_uuid_short(task))
                    ...
                    if self.execute_command(['task', metadata['uuid'], 'modify'] + args, wait=self.wait):
                        self.activate_message_bar(
                            'Task %s modified' % self.model.task_id(metadata['uuid']))
                elif op == 'annotate':
                    task = self.model.task_annotate(
                        metadata['uuid'], data['text'])

                    if task:
                        self.table.flash_focus()
                        self.update_report()
                        self.activate_message_bar(
                            'Annotated task %s' % self.model.task_id(task['uuid']))
                elif op == 'tag':
                    ...
                    if task:
                        ...
                        self.update_report()
                        self.activate_message_bar(
                            'Task %s tags updated' % self.model.task_id(task['uuid']))
        ...
        if 'uuid' in metadata:
            self.task_list.focus_by_task_uuid(
                metadata['uuid'], self.previous_focus_position)

    def focus_new_task(self, task):
        if self.config.get('vit', 'focus_on_add'):
            self.task_list.focus_by_task_uuid(
                task['uuid'], self.previous_focus_position)
    ...
    def activate_help(self, args):
        [self.original_header_top_contents,
            self.original_header_bottom_contents] = self.header.contents
    ...
    def deactivate_help(self, data):
        self.header.contents[:] = [
            self.original_header_top_contents, self.original_header_bottom_contents]
    ...
    def ex(self, text, metadata):
        ...
        if len(args):
            ...
            if command in ('h', 'help'):
                ...
                self.activate_message_bar(
                    "Report '%s' is non-standard, use ':!w task %s'" % (command, command), 'error')
            else:
                ...
                if matches and 'uuid' in metadata:
                    ...
                    if task:
                        description = re.sub(
                            r'%s' % before, after, task['description'])
                        task = self.model.task_description(
                            metadata['uuid'], description)

                        if task:
                            self.table.flash_focus()
                            self.update_report()
                            self.activate_message_bar(
                                'Task %s description updated' % self.model.task_id(task['uuid']))
    ...
    def search(self, reverse=False):
        ...
        new_focus = self.search_rows(
            self.search_term_active, current_index, reverse)

        if new_focus is None:
            self.activate_message_bar(
                "Pattern not found: %s" % self.search_term_active, 'error')
        else:
    ...
    def search_rows(self, term, start_index=0, reverse=False):
        ...
        start_matches = self.search_row_has_search_term(
            rows[start_index], search_regex)
    ...
    def search_display_message(self, reverse=False):
        self.activate_message_bar("Search %s for: %s" % (
            'reverse' if reverse else 'forward', self.search_term_active))

    def search_loop_warning(self, hit, reverse=False):
        self.activate_message_bar('Search hit %s, continuing at %s' % (
            hit, hit == 'TOP' and 'BOTTOM' or 'TOP'))
    ...
    def build_task_table(self):
        self.table = TaskTable(self.config, self.task_config, self.formatter, self.loop.screen, on_select=self.on_select, event=self.event,
                               action_manager=self.action_manager, request_reply=self.request_reply, markers=self.markers, draw_screen_callback=self.loop.draw_screen)
    ...
    def init_autocomplete(self):
        ...
        ), 'help': self.help.autocomplete_entries(), 'context': context_list})

    def init_command_bar(self):
        abort_backspace = self.config.get('vit', 'abort_backspace')
        self.command_bar = CommandBar(
            autocomplete=self.autocomplete, abort_backspace=abort_backspace, event=self.event)

    def build_frame(self):
        self.status_report = urwid.AttrMap(
            urwid.Text('Welcome to VIT'), 'status')
        self.status_context = urwid.AttrMap(urwid.Text(''), 'status')
        self.status_performance = urwid.AttrMap(
            urwid.Text('', align='center'), 'status')
        self.status_version = urwid.AttrMap(urwid.Text(
            'vit (%s)' % version.VIT, align='center'), 'status')
        self.status_tasks_shown = urwid.AttrMap(
            urwid.Text('', align='right'), 'status')
        self.status_tasks_completed = urwid.AttrMap(
            urwid.Text('', align='right'), 'status')
    ...
    def command_error(self, returncode, error_message):
        if returncode != 0:
            self.activate_message_bar(
                "Command error: %s" % error_message, 'error')
    ...
    def task_done(self, uuid):
        ...
        if task:
            if success:
                ...
                self.update_report()
                self.activate_message_bar(
                    'Task %s marked done' % self.model.task_id(task['uuid']))
            else:
    ...
    def task_delete(self, uuid):
        ...
        if task:
            if success:
                ...
                self.update_report()
                self.activate_message_bar(
                    'Task %s deleted' % self.model.task_id(task['uuid']))
            else:
                self.activate_message_bar('Error: %s' % task, 'error')
    ...
    def task_start_stop(self, uuid):
        ...
        if task:
            if success:
                self.table.flash_focus()
                self.update_report()
                self.activate_message_bar('Task %s %s' % (self.model.task_id(
                    task['uuid']), 'started' if task.active else 'stopped'))
            else:
                self.activate_message_bar('Error: %s' % task, 'error')
    ...
    def task_action_annotate(self):
        ...
        if uuid:
            ...
            self.task_list.focus_by_task_uuid(
                uuid, self.previous_focus_position)
    ...
    def task_action_delete(self):
        ...
        if task:
            if self.confirm:
                self.activate_command_bar('delete', 'Delete task %s? (y/n): ' %
                                          self.model.task_id(uuid), {'uuid': uuid, 'choices': {'y': True}})
            else:
    ...
    def task_action_start_stop(self):
        ...
        if task:
            if self.confirm:
                self.activate_command_bar('start-stop', '%s task %s? (y/n): ' % (
                    task.active and 'Stop' or 'Start', self.model.task_id(uuid)), {'uuid': uuid, 'choices': {'y': True}})
            else:
    ...
    def task_action_done(self):
        ...
        if task:
            if self.confirm:
                self.activate_command_bar('done', 'Mark task %s done? (y/n): ' %
                                          self.model.task_id(uuid), {'uuid': uuid, 'choices': {'y': True}})
            else:
                self.task_done(uuid)
                self.task_list.focus_by_task_uuid(
    ...
    def task_action_priority(self):
        ...
        if uuid:
            ...
            for choice in self.task_config.priority_values:
                ...
                choices[key] = choice
            self.activate_command_bar('priority', 'Priority (%s): ' %
                                      '/'.join(choices), {'uuid': uuid, 'choices': choices})
    ...
    def get_available_task_columns(self):
        returncode, stdout, stderr = self.command.run(
            ['task', '_columns'], capture_output=True)

        if returncode == 0:
            self.available_columns = stdout.split()
        else:
            raise RuntimeError(
                "Error retrieving available task columns: %s" % stderr)

    def refresh_blocking_task_uuids(self):
        returncode, stdout, stderr = self.command.run(
            ['task', 'uuids', '+BLOCKING'], capture_output=True)

        if returncode == 0:
            self.blocking_task_uuids = stdout.split()
        else:
            raise RuntimeError(
                "Error retrieving blocking task UUIDs: %s" % stderr)
    ...
    def setup_autocomplete(self, op):
        ...
        if op in ('filter', 'add', 'modify'):
            ...
            filter_config = copy.deepcopy(
                self.autocomplete.default_filter_config)
            ...
            self.autocomplete.setup(
                callback, filters=filters, filter_config=filter_config)
        elif op in ('project',):
            filters = ('project',)
            filter_config = {
                'project': {
    ...
    def update_status_report(self):
        filtered_report = 'task %s %s' % (
            self.report, ' '.join(self.extra_filters))
    ...
    def update_report(self, report=None):
        ...
        context_filters = self.contexts[self.context]['filter'] if self.context and self.reports[self.report].get(
            'context', 1) else []
        try:
            self.model.update_report(
                self.report, context_filters=context_filters, extra_filters=self.extra_filters)
        ...
        self.denotation_pop_up = DenotationPopupLauncher(
            self.task_list, self.formatter, self.loop.screen, event=self.event, request_reply=self.request_reply, action_manager=self.action_manager)
